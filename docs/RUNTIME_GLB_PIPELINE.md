...
`unsupportedMaterialFeature`; the source-compatible `productionShaders()`
policy can route available intent through package-local shader paths after
preflight confirms availability.
The policy constructor keeps maturity `candidate-only`, target evidence
`not run`, and claimed release targets empty; historical Task 012 iOS Simulator
evidence remains a separate durable `verified locally` record. Physical iOS,
