// FSViewerSheenExtendedPbr is an opt-in, package-local candidate for
// KHR_materials_sheen. It preserves the package's existing core/specular/IOR
// evaluator and adds a distinct Charlie/Conty-Kulla cloth lobe. The candidate
// is not renderer-native or production-ready; physical iOS, Android, and Web
...
// The lighting/resource structure remains adapted from flutter_scene revision
...
vec2 TransformSheenExtendedPbrUv(vec2 uv, vec4 offset_scale, vec4 rotation) {
...
void SheenExtendedPbrSurface(inout MaterialInputs material) {
  ...
  vec2 emissive_uv = TransformSheenExtendedPbrUv(
  ...
  vec3 normal = SheenExtendedPbrGeometricNormal();
...
SheenInputs ReadSheenInputs() {
  vec2 sheen_color_uv = TransformSheenExtendedPbrUv(
      v_texture_coords, sheen_params.sheen_color_uv_offset_scale,
      sheen_params.sheen_color_uv_rotation);
  vec2 sheen_roughness_uv = TransformSheenExtendedPbrUv(
...
vec3 SheenExtendedPbrFresnel(float cosine, vec3 f0, vec3 f90) {
  float grazing = pow(clamp(1.0 - cosine, 0.0, 1.0), 5.0);
  return f0 + (f90 - f0) * grazing;
}

vec3 SheenExtendedPbrFresnelRoughness(float cosine, vec3 f0, vec3 f90,
                                      float roughness) {
...
float SheenCharlieDistribution(float n_dot_h, float sheen_roughness) {
...
float SheenVisibilityFit(float x, float alpha_g) {
...
float SheenCharlieLambda(float cosine, float alpha_g) {
  float x = clamp(abs(cosine), 1e-4, 1.0);
  return x < 0.5
      ? exp(SheenVisibilityFit(x, alpha_g))
      : exp(2.0 * SheenVisibilityFit(0.5, alpha_g) -
            SheenVisibilityFit(1.0 - x, alpha_g));
}

float SheenCharlieVisibility(float n_dot_v, float n_dot_l,
  ...
  float denominator =
      (1.0 + SheenCharlieLambda(safe_n_dot_v, alpha_g) +
       SheenCharlieLambda(safe_n_dot_l, alpha_g)) *
      (4.0 * safe_n_dot_v * safe_n_dot_l);
  return clamp(1.0 / max(denominator, 1e-6), 0.0, 1.0);
}

float SheenDirectionalAlbedo(float n_dot_x, float sheen_roughness) {
  vec2 lut_uv = clamp(
      vec2(n_dot_x, max(sheen_roughness, 0.07)), vec2(0.0), vec2(0.99));
  return texture(brdf_lut, lut_uv).b;
}

// Exact zero-sheen path: this is the pre-existing FSViewerExtendedPbr
// evaluator with only local symbol renames. It deliberately does not sample
// Charlie directional albedo or alter base roughness/environment intensity.
vec4 EvaluateSheenZeroLighting(MaterialInputs material) {
  vec3 albedo = material.base_color.rgb;
  float alpha = material.base_color.a;
  vec3 normal = material.normal;
  vec3 geometric_normal = SheenExtendedPbrGeometricNormal();
  ...
  vec3 k_S = SheenExtendedPbrFresnelRoughness(
  ...
  if (frag_info.has_directional_light > 0.5 && n_dot_l > 0.0) {
    ...
    vec3 specular_fresnel = SheenExtendedPbrFresnel(
...
vec4 EvaluateSheenLayeredLighting(MaterialInputs material, SheenInputs sheen) {
  float max_sheen_color = max(max(sheen.color.r, sheen.color.g), sheen.color.b);
  if (max_sheen_color <= 0.0) {
    return EvaluateSheenZeroLighting(material);
  }
  ...
  float sheen_energy_v =
      SheenDirectionalAlbedo(n_dot_v, sheen_roughness);
  ...
  if (frag_info.has_directional_light > 0.5 && n_dot_l > 0.0) {
    ...
    float sheen_distribution =
        SheenCharlieDistribution(n_dot_h, sheen_roughness);
    float sheen_visibility =
        SheenCharlieVisibility(n_dot_v_safe, n_dot_l, sheen_roughness);
    vec3 direct_sheen =
        sheen.color * sheen_distribution * sheen_visibility;
    float sheen_energy_l =
        SheenDirectionalAlbedo(max(n_dot_l, 0.0), sheen_roughness);
...
void main() {
  ...
  SheenExtendedPbrSurface(material);
  SheenInputs sheen = ReadSheenInputs();
  frag_color = EvaluateSheenLayeredLighting(material, sheen);
}
